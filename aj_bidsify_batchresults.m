%----------------------------------------------------------------------------------------------------
% Script to BIDSify files after running a MATLAB batch process (e.g., preprocessing steps)
% 
% Purpose: The script identifies and organizes new files created during preprocessing 
%    (typically named with prefixes like r*, w*, mean*) into a derivative directory
%    according to the BIDS standards.
%----------------------------------------------------------------------------------------------------
% Workflow Overview:
% 1. Identify New Files:
%    - The script compares the original BIDS dataset (`orig_bids_root`) with a working copy 
%      of the BIDS dataset (`work_bids_root`) to identify newly created files.
%    - New files are detected by comparing the base names (filename without extensions) between
%      the two datasets for each type of data (e.g., `func`, `anat`, `fmap`, `dwi`).
%
% 2. Move Files to Derivatives:
%    - Once new files are identified, they are moved into a `derivatives/preprocessing` folder,
%      following the BIDS structure. The `derivatives` folder is created if it doesn't exist.
%    - Each data type (e.g., `anat`, `func`, etc.) has its own sub-folder under `derivatives/preprocessing`,
%      ensuring a clear organization of the newly created files.
%
% 3. Validation Before Deletion:
%    - After all new files are moved, the script checks if the number of moved files matches
%      the number of new files detected.
%    - If the number matches (i.e., all files were successfully moved), the script deletes
%      the corresponding files from the working BIDS dataset (`work_bids_root`), cleaning up the workspace.
%
% 4. Limitation with Similar File Names:
%    - If two files have the same name but different extensions (e.g., `file.nii` and `file.nii.gz`), 
%      the current method of detecting new files (by comparing base names) may not treat them as distinct files. 
%      As a result, such files may not be correctly identified as new files and may remain in the working dataset.
%
%    - This limitation exists because the comparison only looks at the file name without extensions.
%      In future updates, this issue could be addressed by extending the comparison to include both 
%      base names and extensions.
%----------------------------------------------------------------------------------------------------

function aj_bidsify_batchresults(orig_bids_root, work_bids_root, subject, session)
%     session = 'ses-mri';
...
%     addpath('C:\Users\antoi\Documents\master_thesis\MATLAB\spm12');
    ...
    for i = 1:numel(valid_data_types)
        ...
        if ~isempty(subject_info.(data_type))
            ...
            orig_content = dir(fullfile(orig_bids_root, subject, session, data_type));
            work_content = dir(fullfile(work_bids_root, subject, session, data_type));
            ...
            if ~isempty(new_files)
                successful_move = move_files_to_bids_structure(work_bids_root, subject, session, data_type, new_files);
                
                % After confirming all files were successfully moved, delete from work directory
                if successful_move
                    delete_new_files_in_work(work_bids_root, subject, session, data_type, new_files);
                end
            end
        else
